...
from gambitpairing.core.update_worker import ModernUpdateWorker
...
class GambitPairingMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.updater: Optional[ModernUpdater] = ModernUpdater(APP_VERSION)
    ...
    def prompt_update(self):
        ...
        if not self.updater or not self.updater.latest_release:
            return

        latest_version = self.updater.get_latest_version()
        release_notes = self.updater.get_release_notes()

        if not latest_version or not release_notes:
            QtWidgets.QMessageBox.warning(
    ...
    def start_update_download(self):
        ...
        self.worker = ModernUpdateWorker(self.updater)
